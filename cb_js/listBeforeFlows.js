(function(cmd, context) {
    ...
    context.ux.action.start('Calculating Before Flows');
    return context.connection.query(`SELECT DurableId FROM EntityDefinition WHERE QualifiedApiName = '${objectApiName}'`).then(data => {
        ...
        return context.connection.query(`SELECT DurableId, ApiName, NamespacePrefix, RecordTriggerType FROM FlowDefinitionView WHERE IsActive = true AND TriggerObjectOrEventId = '${durableId}' AND (TriggerType = 'RecordBeforeSave' OR TriggerType = 'RecordBeforeDelete') ORDER BY TriggerOrder ASC`).then(data => {
