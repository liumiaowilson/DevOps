(function(cmd, context) {
    const SOQL = "SELECT Id FROM Item__c WHERE Type__c = 'Beauty'";
    ...
    return (async () => {
        context.ux.action.start('Querying Beauty Item__c records');
        const records = await queryAll();
        context.ux.action.stop();

        if(!records.length) {
            cmd.logSuccess('No Beauty records found');
            return;
        }
        ...
        cmd.logSuccess('Marked ' + updates.length + ' Beauty records as unprocessed');
    })().catch(err => {
        context.ux.action.stop();
