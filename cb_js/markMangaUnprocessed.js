(function(cmd, context) {
    const SOQL = "SELECT Id FROM Item__c WHERE Type__c = 'Manga'";
    ...
    return (async () => {
        context.ux.action.start('Querying Manga Item__c records');
        const records = await queryAll();
        context.ux.action.stop();

        if(!records.length) {
            cmd.logSuccess('No Manga records found');
            return;
        }
        ...
        cmd.logSuccess('Marked ' + updates.length + ' Manga records as unprocessed');
    })().catch(err => {
        context.ux.action.stop();
