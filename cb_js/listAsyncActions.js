(function(cmd, context) {
    ...
    context.ux.action.start('Calculating Async Actions');
    return context.connection.query(`SELECT DurableId FROM EntityDefinition WHERE QualifiedApiName = '${objectApiName}'`).then(data => {
        ...
        return context.connection.query(`SELECT DurableId, ApiName, NamespacePrefix FROM FlowDefinitionView WHERE IsActive = true AND TriggerObjectOrEventId = '${durableId}' AND HasAsyncAfterCommitPath = true ORDER BY TriggerOrder ASC`).then(data => {
            ...
            cmd.styledHeader('# Async Actions');
