(function(cmd, context) {
    const SOQL = "SELECT Id FROM Item__c WHERE Type__c = 'Cartoon'";
    ...
    return (async () => {
        context.ux.action.start('Querying Cartoon Item__c records');
        const records = await queryAll();
        context.ux.action.stop();

        if(!records.length) {
            cmd.logSuccess('No Cartoon records found');
            return;
        }
        ...
        cmd.logSuccess('Marked ' + updates.length + ' Cartoon records as unprocessed');
    })().catch(err => {
        context.ux.action.stop();
