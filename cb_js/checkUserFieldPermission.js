(function(cmd, context) {
    ...
    const fieldName = context.argv[2];
    if(!fieldName) {
    ...
    return Promise.all([
        context.connection.sobject('FieldPermissions').describe(),
    ]).then(([ fieldPermissionsDescribe ]) => {
        const permissionNames = fieldPermissionsDescribe.fields
        ...
        return Promise.all([
            ...
            context.connection.query(`SELECT ParentId, ${permissionNames.join(', ')} FROM FieldPermissions WHERE SobjectType = '${objectApiName}' AND Field = '${objectApiName}.${fieldName}'`),
        ]).then(([ psData, mpsData, psgData, psgcData, psaData, fpData ]) => {
            const result = [];
            ...
            for(const fp of fpData.records) {
                const ps = psMap[fp.ParentId];
                if(ps) {
                    for(const permissionName of permissionNames) {
                        ps[permissionName] = fp[permissionName];
                    }
                }

                const mps = mpsMap[fp.ParentId];
                if(mps) {
                    for(const permissionName of permissionNames) {
                        mps[permissionName] = fp[permissionName];
                    }
                }
            }
